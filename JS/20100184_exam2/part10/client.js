...
socket.on('begin2', data =>{
	console.log(data)
	if(data){
		setState({msg: `you are player ${data}`,begin: true})
	} else{
		setState({msg: "validation error"})
	}
})

socket.on('wait', (data) =>{ 
	setState({msg: "awaiting opponent", begin: true})
})
...
const beginGame = (ev) =>{
	...
	if(!state.begin){
		console.log('begin')
		socket.emit('begin', [state.board, shipsize])
	}
}
const setState = updates => {
    ...
    ReactDOM.render(
    	React.createElement('div', null,
    		...
    		React.createElement('div', {onClick: ev=> beginGame(ev)},
...
make_bd()
setState({msg: 'Hello World', board: bd, begin: false})
